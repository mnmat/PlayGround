// -*- C++ -*-
//
// Package:    PlayGround/HGCRefitAnalyzer
// Class:      HGCRefitAnalyzer
//
/**\class HGCRefitAnalyzer HGCRefitAnalyzer.cc PlayGround/HGCRefitAnalyzer/plugins/HGCRefitAnalyzer.cc
...
class HGCRefitAnalyzer : public edm::one::EDAnalyzer<edm::one::SharedResources> {
public:
  explicit HGCRefitAnalyzer(const edm::ParameterSet&);
  ~HGCRefitAnalyzer() override;
...
//
HGCRefitAnalyzer::HGCRefitAnalyzer(const edm::ParameterSet& iConfig)
...
HGCRefitAnalyzer::~HGCRefitAnalyzer() {
...
void HGCRefitAnalyzer::printTrackParameters(const reco::Track& track) const{
...
void HGCRefitAnalyzer::printTrackExtraParameters(const reco::Track& track) const{
...
void HGCRefitAnalyzer::mergeRecHitCollections(std::vector<const HGCRecHit*>& recHitCollection,
                                const HGCRecHitCollection& rechitsEE,
                                const HGCRecHitCollection& rechitsFH,
                                const HGCRecHitCollection& rechitsBH) const {
...
TransientTrackingRecHit::ConstRecHitContainer HGCRefitAnalyzer::getTransientRecHits(
    const reco::TransientTrack& track) const {
...
void HGCRefitAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for (const auto& track : iEvent.get(tracksToken_)) {
    ...
    */
    ...
    for (int i = 0; i<int(recHitsforReFit.size());i++){
      std::cout << res.residualX(i) << std::endl; 
    }
...
void HGCRefitAnalyzer::beginJob() {
  // please remove this method if not needed
}

// ------------ method called once each job just after ending the event loop  ------------
void HGCRefitAnalyzer::endJob() {
  // please remove this method if not needed
}

// ------------ method fills 'descriptions' with the allowed parameters for the module  ------------
void HGCRefitAnalyzer::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
DEFINE_FWK_MODULE(HGCRefitAnalyzer);
